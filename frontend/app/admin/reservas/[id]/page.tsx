...
export default function DetalleReservaPage() {
    ...
    return (
        <main className="max-w-7xl mx-auto p-6 flex flex-col font-sans bg-gray-50 min-h-screen relative print:p-0 print:bg-white">
            ...
            {/* 🟢 TICKET DE IMPRESIÓN IMPRIMIBLE (80mm) */}
      ...
      `}} />
            <div className="hidden print:block w-[78mm] mx-auto m-0 p-4 font-sans text-black bg-white leading-tight">
                ...
                <table className="w-full mb-4 border-b-2 border-dashed border-black pb-3 text-[12px]">
                    ...
                    <tbody className="pt-2 block w-full" style={{ display: 'table-row-group' }}>
                        {precioCancha > 0 && (
                            <tr>
                                <td className="py-1 pr-2">1x ALQUILER CANCHA</td>
                                <td className="py-1 text-right font-bold text-[13px] whitespace-nowrap">${precioCancha.toLocaleString()}</td>
                            </tr>
                        )}
                        {reserva.consumos.filter(c => !c.producto.startsWith('Alquiler') && !c.producto.startsWith('✅ PAGO')).map(c => (
                            <tr key={c.id}>
                                <td className="py-1 uppercase pr-2">{c.cantidad}x {c.producto}</td>
                                <td className="py-1 text-right font-bold text-[13px] whitespace-nowrap">${c.precio.toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <table className="w-full text-[13px]">
                    <tbody>
                        <tr>
                            <td className="py-1 text-left font-bold">SUBTOTAL</td>
                            <td className="py-1 text-right font-bold">${totalBruto.toLocaleString()}</td>
                        </tr>
                        {descuentoGlobal > 0 && (
                            <tr className="text-[12px]">
                                <td className="py-1 text-left font-bold">DESCUENTO GLOBAL</td>
                                <td className="py-1 text-right font-bold">- ${descuentoGlobal.toLocaleString()}</td>
                            </tr>
                        )}
                        <tr className="text-[18px] font-black">
                            <td className="pt-3 pb-2 text-left uppercase">Total a Pagar</td>
                            <td className="pt-3 pb-2 text-right">${totalGeneral.toLocaleString()}</td>
                        </tr>

                        <tr className="text-[12px] border-t border-dashed border-gray-400">
                            <td className="pt-3 text-left">Total Abonado</td>
                            <td className="pt-3 text-right font-bold">${totalPagado.toLocaleString()}</td>
                        </tr>
                        <tr className="text-[13px] font-bold">
                            <td className="py-1 text-left uppercase">{saldoPendiente > 0 ? 'Resta Pagar' : (saldoPendiente < 0 ? 'Vuelto' : 'Saldo')}</td>
                            <td className="py-1 text-right uppercase">${Math.abs(saldoPendiente).toLocaleString()}</td>
                        </tr>
                    </tbody>
                </table>
